...
contract Data {
    ...
    function transferOwnership_data(uint user1_id ,uint user2_id, uint data_id) onlyOwner(data_id) public returns(bool) {
        ...
        (keccak256(p1._userType) == keccak256("CurrentOwner") && keccak256(p2._userType)==keccak256("CurrentOwner"))
            ...
            tracks[track_id]._data_id =data_id;
            tracks[track_id]._owner_id = user2_id;
            tracks[track_id]._owner_name=Users[user2_id]._userName;
            tracks[track_id]._timeStamp = now;
            tracks[track_id]._previous_owner_id = user1_id;
            tracks[track_id]._previous_owner_name=Users[user1_id]._userName;
            datas[data_id]._owner_id = user2_id;
            datas[data_id].trace_ids.push(track_id);
...
function getdetails(uint dataid,uint owner_id, uint selectedOwnerid,uint buyerId) public returns (uint){
    ...
    function concatenateInfoAndHash(string memory s1,bytes memory s2,string memory s3,bytes memory s4) private returns (bytes32){
